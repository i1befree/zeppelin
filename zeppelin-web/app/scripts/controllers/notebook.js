...
//		]	
  ...
  /** getParagraph filter */
  $scope.refreshDependencyFilteredParagraphs = function(paragraphId, dataFilter) {
    for (var i=0; i<$scope.note.paragraphs.length; i++) {
      if ($scope.note.paragraphs[i].config.dependencyParagraph !== undefined) {
        angular.forEach($scope.note.paragraphs[i].config.dependencyParagraph, function(item, index) {
          if(item.id === paragraphId) {
            console.info($scope.note.paragraphs[i].id, item.id , paragraphId, dataFilter);
            $scope.$broadcast('dependencyFilteredParagraph', $scope.note.paragraphs[i].id, dataFilter);
          }
        });
      }
    }
  ...
  $scope.$on('setNoteContent', function(event, note) {
    if(note.userId != Authentication.getId()) {
      return;
    }
  ...
  var cleanParagraphExcept = function(paragraphId, note) {
    ...
    for (var i=0; i<note.paragraphs.length; i++) {
      if (note.paragraphs[i].id === paragraphId) {
        ...
        noteCopy.paragraphs[0].config.master = true;
        break;
      }
    }
    return noteCopy;
