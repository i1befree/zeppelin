...
drop database trip;
use mysql;
delete from user where User = 'trip';  
commit;
...
/* 초기데이타 세팅 */
INSERT INTO `trip`.`user` (`id`,`name`,`passwd`,`email`,`tel`,`user_grp_cd`,`create_date`,`update_date`) VALUES ('admin','관리자',sha1('cepadmin'),NULL,NULL,'1',NULL,NULL);
...
INSERT INTO `trip`.`workspace` (`workspace_id`,`name`,`type`,`p_id`) VALUES ('15634a90-0e8f-44e4-98e1-550be090a210','Personal','P','ROOT');
INSERT INTO `trip`.`workspace` (`workspace_id`,`name`,`type`,`p_id`) VALUES ('f5697b8a-6fe0-4d0e-be26-3472b3c8e2eb','Shared','S','ROOT');
INSERT INTO `trip`.`workspace` (`workspace_id`,`name`,`type`,`p_id`) VALUES ('79efbe48-8205-4a1c-91e3-051bb246b468','Global','G','ROOT');

DROP TABLE IF EXISTS `trip`.`notebook_workspace`;
CREATE TABLE `trip`.`notebook_workspace` (
  `note_id` varchar(100) NOT NULL COMMENT '노트아이디',
  `workspace_id` varchar(36) NOT NULL COMMENT '작업공간아이디:(UUID)',
