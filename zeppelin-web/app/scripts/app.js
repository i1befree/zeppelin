...
function getReqBaseUrl(uriPath) {
  var port = Number(location.port);
  if (port === 'undefined' || port === 0) {
    port = 80;
    if (location.protocol === 'https:') {
  ...
  return location.protocol+"//"+location.hostname+":"+port+(uriPath == undefined ? '/cep' : uriPath);
}
...
angular
  ...
  .config(function ($routeProvider, WebSocketProvider, $httpProvider) {
    WebSocketProvider
      .prefix('')
